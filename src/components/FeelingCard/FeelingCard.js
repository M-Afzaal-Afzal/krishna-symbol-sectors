...
const FeelingCard = () => {
  ...
  const [reactionToSymbol, setReactionToSymbol] = useState(localStorage.getItem(`${symbol}reaction`));

  const [selectedTopVolumeData, setSelectedTopVolumeData] = useState([]);

  const reactionDate = +localStorage.getItem('rcDate');

  const reactionToSymbolDate = localStorage.getItem(`${symbol}rcDate`);
  ...
  const localStorageStateHandler = (reaction) => {
    const today = new Date();
    const date = today.getDate();
    localStorage.setItem(`${symbol}rcDate`, `${date}`);
    localStorage.setItem(`${symbol}reaction`, reaction);
  };
  ...
  },[goodPercentageForState,overallRatingData,stateRatingData])
    ...
    if (stateRatingStatus !== 'loading') {
      ...
      if (!stateRatingData) {
        firestore
          ...
          .then((value) => {
            localStorageStateHandler(`${symbol}happy`);
            setReactionToSymbol(`${symbol}happy`);
            console.log(value, 'Document updated successfully');
          });
      } else {
    ...
    if (stateRatingStatus !== 'loading') {
      ...
      if (!stateRatingData) {
        firestore
          ...
          .then((value) => {
            localStorageStateHandler(`${symbol}sad`);
            setReactionToSymbol(`${symbol}sad`);
            console.log(value, 'Document updated successfully');
          });
      } else {
  ...
  useEffect(() => {
    ...
    if (!token || (token && time >= tokenExpiryTime)) {
      client.post('/auth/local', {
        ...
        .catch(err => {
          console.log(err.message,'From feeling card');
        });
    } else {
  ...
  return (
    <Box>
      ...
      <Box py={'2rem'} px={'1rem'} w={'100%'} bg={value ? '#161625' : '#fff'}>
        ...
        {/*-------------------------------------------------*/}
        {/* ------------ for symbol is available ----------- */}
        {/*-------------------------------------------------*/}
        {
          symbol && (
            (!reactionToSymbol) || (reactionToSymbol && (reactionToSymbolDate < todayDate)) ? (
              <>
                <Heading fontFamily={'archia, sans-serif'} color={value ? '#fff' : 'gray.500'} fontWeight={400}
                         fontSize={'1.3rem !important'} textAlign={'center'}>
                  How do you feel about celsius network today?
